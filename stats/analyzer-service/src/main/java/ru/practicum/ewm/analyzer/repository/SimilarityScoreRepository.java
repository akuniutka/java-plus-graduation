...
public interface SimilarityScoreRepository extends JpaRepository<SimilarityScore, Long> {
    ...
    @Query("""
            select s.eventBId as eventId, s.score as score
    ...
    @Query("""
            select s.eventBId as eventId, max(s.score) as score
            from SimilarityScore as s
    ...
    @Query("""
            select s.eventAId as eventId, sum(u.score * s.score) / sum(s.score) as score
            from SimilarityScore as s
